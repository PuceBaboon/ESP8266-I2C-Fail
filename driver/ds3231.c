/*
 * $Id: ds3231.c,v 1.6 2015/07/25 03:06:54 anoncvs Exp $
 *
 * Note that struct tm defines everything as "int", which by default on the
 * ESP8266 is a 32-bit value.
 */
...
/*
 * Test Time and Date memory registers in the target DS3231 by writing a hex
 * pattern and doing an immediate read and compare.
 * Only needed as a test tool to verify the chip.
 * == DO NOT LEAVE THIS RUNNING AD INFINITUM ==     ...once or twice is enough.
 */
void ICACHE_FLASH_ATTR ds3231_test_tdmem() {
	static	const uint8_t 	tpatt[]	= {0xFF, 0xAA, 0x55, 0x00};	// Test patterns.
	static const uint8_t	tpmask[] = {0x7F, 0x7F, 0x7F, 0x07, 0x3F, 0x9F, 0xFF};
	uint8_t			data[8];				// Buffer for data transfers.
	uint8_t			i, x;

	INFO("------------------------------------------------------------\n");
	INFO("Running Time/Date Register Memory Test\n");
	for (i=0; i<=3; i++) {
		// Start address of time registers.
		data[0]	= DS3231_ADDR_TIME;

		// Test data pattern.
		data[1] = data[2] = data[3] = data[4] = data[5] = data[6] = data[7] = tpatt[i];

		if (! ds3231_send(data, 8)) {
			INFO("==> Load of test data failed!\n");
			return;
		}
		INFO("\t\tPatt: %02X written.\n", tpatt[i]);

		// Clear our data buffer.
		for (x=0; x<=7; x++) {
			if (tpatt[i] == 0X00) {
				data[x] = 0xFF;
			} else {
		...
		// Re-send start address at beginning of new read operation.
		data[0] = DS3231_ADDR_TIME;
		if (! ds3231_send(data, 1)) {
			INFO("==> Set read address failed!\n");
			return;
		}

		// Now read back the previously written data into the buffer.
		if (! ds3231_recv(data, 7)) {
			INFO("==> Read data operation failed!\n");
			return;
		}
		INFO("Reading ");

		for (x=0; x<=6; x++) {
			if (data[x] != (tpatt[i] & tpmask[x])) {
				INFO("==> MEMORY FAIL.  Byte: %02X\tWrote: %02X\tRead: %02X\tMask: %02X\tMskData: %02X\n",
						x, tpatt[i], data[x], tpmask[x], (tpatt[i] & tpmask[x]));
				return;
			} else {
				INFO(".");
			}
	...
	INFO("Time/Date Memory register test successfully completed.\n");
	INFO("------------------------------------------------------------\n");
	return;
}
...
INFO("RTC Set Yr: %02X\t%d\n", decToBcd(time->tm_year), time->tm_year); 
INFO("RTC Set Yr: %02X\t%d\n", decToBcd(time->tm_year - 100), time->tm_year - 100); 
...
bool ICACHE_FLASH_ATTR ds3231_clearAlarmFlags(uint8 alarms) {
	return ds3231_setFlag(DS3231_ADDR_STATUS, alarms, DS3231_CLEAR);	// DS3231_CLEAR is setFlag() instruction, not a value!!
}

// Added 22nd June 2015.  J.L.
//void ICACHE_FLASH_ATTR ds3231_turnOffAlarm(uint8_t Alarm) {
//	// turns off alarm number "Alarm". Defaults to 2 if Alarm is not 1.
//	// Leaves interrupt pin alone.
//	uint8	temp_buffer = readControlByte(DS3231_ADDR_STATUS);
//	ds3231_getFlag(DS3231_ADDR_CONTROL, DS3231_ALARM_BOTH, &temp_buffer);
//	// modify control byte
//	if (Alarm == 1) {
...
//	}
//	ds3231_setFlag(DS3231_ADDR_CONTROL, DS3231_ALARM_BOTH, temp_buffer);
//	writeControlByte(temp_buffer, DS3231_ADDR_STATUS);
//}
...
// Get the temperature as two bytes; the whole number integer
// part and the fractional part.  The fractional part is encoded
// in two bits of the second byte (so precision is limited to 
// 1/4 of a degree C).
// Returns true on success, false on failure.
bool ICACHE_FLASH_ATTR ds3231_getTempBytes(int8 *tempMSB, int8 *tempLSB) {
	uint8 data[2];
	data[0] = DS3231_ADDR_TEMP;
	// get just the integer part of the temp
...
// returns true to indicate success
//
// ==NOTE== This function doesn't populate "yday", so conversions
// using standard gmtime()/localtime() will not work.
//
